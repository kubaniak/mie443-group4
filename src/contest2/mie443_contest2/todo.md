...
- What we want:
    ...
    - (DONE) Implemented single-use detections for scene classes (cup, motorcycle, clock, plant, water bottle) so duplicate boxes aren't re-logged as found objects.

    - Optional: Retry if time remaining, only visiting locations where no object was yolo'd (the `remaining_scene_objects` logic can be extended for this)
